...
namespace GetGroups
{
    class GetGroups
    {
        static void Main(string[] args)
        {
            ...
            // REST URI
            var url = "https://ct.soa-gw.canadapost.ca/rs/" + mailedBy + "/" + mobo + "/group";
            ...
            try
            {
                ...
                // Deserialize xml response to GroupType object
                XmlSerializer serializer = new XmlSerializer(typeof(GroupsType));
                TextReader reader = new StreamReader(response.GetResponseStream());
                GroupsType groups = (GroupsType)serializer.Deserialize(reader);

                // Retrieve values from GroupType object
                if (groups.group != null)
                {
                    foreach (GroupType group in groups.group)
                    {
